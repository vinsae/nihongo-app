...
function speak(text) {
  ...
  const u = new SpeechSynthesisUtterance(text.replace(/[⚠️（）()「」【】]/g,'').split(' ')[0])
  u.lang = 'ja-JP'; u.rate = 0.85
  window.speechSynthesis.speak(u)
}

// Modules that have a quiz
...
export default function ModulePage({ mod, progress, onBack, onSaveProgress }) {
  ...
  // "Mark complete" for non-quiz modules (budaya, bahasa-kerja, counter, verb-advanced)
  const handleMarkDone = () => onSaveProgress(mod.id, 100, 50)
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#0b0f1a', color:'#f0ece4', fontFamily:'"DM Sans",sans-serif' }}>
      <header style={{ borderBottom:'1px solid rgba(255,255,255,0.06)', padding:'14px 24px', display:'flex', alignItems:'center', gap:16, position:'sticky', top:0, background:'#0b0f1a', zIndex:10 }}>
        ...
        {isDone && <div style={{ fontSize:12, color:'#2a9d8f', fontWeight:600 }}>✓ Selesai</div>}
      </header>
...
function HiraganaLearn({ c }) {
  ...
  const LABELS = { base:'46 Dasar', dakuten:'+ Dakuten (voiced)', combo:'+ Kombinasi' }
  const DESCS = {
    base:    'あ い う え お — 46 huruf inti. Hafal baris per baris dari atas ke bawah.',
    dakuten: 'Huruf bersuara: が(ga) ざ(za) だ(da) ば(ba) dan semi-suara ぱ(pa).',
    combo:   'Kombinasi 2 huruf: きゃ(kya) しゃ(sha) ちゃ(cha) dll — huruf kecil や ゆ よ.',
  }
  const groups = [...new Set(DATA[sec].map(h => h.group))]
  return (
    <div>
      <InfoBox color={c} text="Klik kartu untuk flip dan lihat bacaan romaji. Klik lagi untuk balik." />
      ...
      <TipBox color={c} title="Tips" text="Mulai baris あ → か → さ → た → な → は → ま → や → ら → わ → ん. Target: 5 huruf per hari = hafal semua dalam ~2 minggu!" />
    </div>
  )
}
...
function KatakanaLearn({ c }) {
  ...
  const DESCS = {
    base:    'Bunyi identik Hiragana, bentuk huruf berbeda. Dipakai untuk kata serapan & nama asing.',
    dakuten: 'Versi bersuara Katakana: ガ ザ ダ バ dan semi-suara パ.',
    combo:   'キャ シャ チャ dll — sangat sering muncul pada kata serapan Barat.',
  }
  const groups = [...new Set(DATA[sec].map(k => k.group))]
  return (
    <div>
      <InfoBox color={c} text="Contoh kata: テレビ(TV) コーヒー(kopi) スマートフォン(smartphone) アイスクリーム(es krim)" />
      <SegBtns items={LABELS} active={sec} onChange={setSec} c={c} />
      <p style={{ fontSize:13, color:'#8890a4', marginBottom:20 }}>{DESCS[sec]}</p>
      {groups.map(g => (
        <div key={g} style={{ marginBottom:20 }}>
      ...
      <TipBox color={c} title="Tips" text="Setelah hafal Hiragana, Katakana jadi lebih cepat. Coba baca nama produk/merek di kemasan Jepang setiap hari!" />
    </div>
  )
}
    ...
    <div>
      <div style={{ display:'flex', gap:8, flexWrap:'wrap', marginBottom:20 }}>
        {Object.entries(VOCAB_CATS).map(([k,label]) => (
          <button key={k} onClick={()=>setCat(k)} style={{ padding:'6px 14px', borderRadius:20, fontSize:12, fontWeight:500, cursor:'pointer', border:'1px solid rgba(255,255,255,0.1)', background:cat===k?c:'transparent', color:cat===k?'#fff':'rgba(255,255,255,0.5)', transition:'all 0.2s', whiteSpace:'nowrap' }}>{label}</button>
        ))}
      </div>

      {/* Special: Numbers system explanation */}
      {cat === 'numbers' && (
        <div style={{ background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.08)', borderRadius:10, padding:'14px 16px', marginBottom:16, fontSize:13, color:'rgba(255,255,255,0.7)', lineHeight:1.9 }}>
          <strong style={{ color:c }}>📐 Sistem Angka Jepang:</strong><br/>
          <code style={{ color:'#f4a261' }}>11 = じゅう + いち = じゅういち</code><br/>
          <code style={{ color:'#f4a261' }}>25 = に + じゅう + ご = にじゅうご</code><br/>
          <code style={{ color:'#f4a261' }}>135 = ひゃく + さん + じゅう + ご = ひゃくさんじゅうご</code><br/>
          <span style={{ color:'#e63946', fontSize:12 }}>⚠️ Angka tidak beraturan: 300(さんびゃく) 600(ろっぴゃく) 800(はっぴゃく) 3000(さんぜん)</span>
        </div>
      )}

      <InfoBox color={c} text="Klik kartu untuk lihat arti. Klik 🔊 untuk dengar pengucapan." />
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(175px,1fr))', gap:10, marginTop:12 }}>
...
function GrammarLearn({ c }) {
  ...
  const labels = Object.fromEntries(GRAMMAR.map(g => [g.id, g.icon+' '+g.title]))
  return (
    <div>
      <SegBtns items={labels} active={GRAMMAR[idx].id} onChange={v=>setIdx(GRAMMAR.findIndex(g=>g.id===v))} c={c} />
      <div style={{ display:'grid', gap:10 }}>
        {GRAMMAR[idx].items.map((item,i) => (
          <div key={i} style={{ background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:10, padding:'14px 16px' }}>
            ...
            {item.ex && (
              <div style={{ background:'rgba(255,255,255,0.04)', borderRadius:8, padding:'10px 14px', borderLeft:`2px solid ${c}60`, cursor:'pointer' }} onClick={()=>speak(item.ex)}>
                <div style={{ fontSize:16, fontFamily:'"Noto Serif JP",serif', marginBottom:4 }}>{item.ex} <span style={{ fontSize:12, color:c }}>🔊</span></div>
                <div style={{ fontSize:12, color:'rgba(255,255,255,0.5)' }}>{item.tr}</div>
              </div>
            )}
            {item.tip && <div style={{ fontSize:12, color:'#f4a261', marginTop:8 }}>💡 {item.tip}</div>}
          </div>
        ))}
      </div>
    </div>
...
function KanjiLearn({ c }) {
  ...
  const filtered = KANJI_N5.filter(k => k.k.includes(q)||k.m.toLowerCase().includes(q.toLowerCase())||k.on.includes(q)||k.kun.includes(q))
  return (
    <div>
      <InfoBox color={c} text="Klik kartu untuk lihat arti, on-yomi, kun-yomi, dan contoh kata. Target: 5 kanji per hari!" />
      <input placeholder="Cari kanji, arti, atau bacaan…" value={q} onChange={e=>setQ(e.target.value)} style={{ width:'100%', maxWidth:300, padding:'9px 14px', borderRadius:8, fontSize:13, background:'rgba(255,255,255,0.06)', border:'1px solid rgba(255,255,255,0.1)', color:'#f0ece4', outline:'none', marginBottom:20, fontFamily:'"DM Sans",sans-serif' }} />
      ...
      <TipBox color={c} title="Tips Kanji" text="Fokus makna dulu sebelum cara baca. Visualisasikan: 山 = puncak gunung, 川 = aliran sungai. Tulis berulang sambil ucapkan artinya." />
    </div>
  )
}
...
function ConversationLearn({ c }) {
  ...
  return (
    <div>
      ...
      <InfoBox color={cv.color} text="Klik 🔊 untuk dengar. Latih teknik shadowing: dengar → tirukan langsung tanpa jeda." />
...
function BudayaLearn({ c }) {
  ...
  return (
    <div>
      ...
      <InfoBox color={c} text="Memahami budaya kerja Jepang sama pentingnya dengan bahasa. Ini kunci sukses bekerja di sana!" />
      <div style={{ display:'grid', gap:10 }}>
        {sec.items.map((item,i) => (
          <div key={i} style={{ background:'rgba(255,255,255,0.04)', borderLeft:`3px solid ${c}`, borderRadius:'0 10px 10px 0', padding:'14px 16px' }}>
            ...
            <div style={{ fontSize:13, fontWeight:500, marginBottom:item.tip?6:0 }}>{item.id}</div>
...
function BahasaKerjaLearn({ c }) {
  ...
  return (
    <div>
      ...
      <InfoBox color={sec.color} text="Hafalkan terutama bagian Keselamatan Kerja — wajib tahu sebelum bekerja di pabrik Jepang!" />
      <div style={{ display:'grid', gap:10 }}>
        {sec.items.map((item,i) => (
    ...
    <div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(120px,1fr))', gap:10 }}>
        {cnt.examples.map((ex,i) => (
          ...
          >
            <div style={{ fontSize:12, color:'rgba(255,255,255,0.35)', marginBottom:4 }}>{ex.n}</div>
      ...
      <TipBox color={c} title="Tips" text="Mulai hafal 〜つ (hitotsu~too) karena bisa dipakai untuk benda apapun. Perhatikan perubahan bunyi: 1本=いっぽん, 3本=さんぼん, 6本=ろっぽん." />
    </div>
  )
}
...
function JlptInfoLearn({ c }) {
  return (
    <div>
      <InfoBox color={c} text="Pelajari materi di sini lalu gunakan tab Quiz untuk latihan soal format JLPT N5!" />
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(200px,1fr))', gap:14, marginBottom:24 }}>
        {[
          { icon:'📖', title:'Moji-Goi', desc:'Membaca Kanji & Kosakata', count:'8 soal', color:'#e63946' },
          { icon:'📝', title:'Bunpou', desc:'Partikel & Tata Bahasa', count:'8 soal', color:'#457b9d' },
          { icon:'💬', title:'Goi (Vocab)', desc:'Kata Sifat & Kosakata Umum', count:'8 soal', color:'#2a9d8f' },
        ].map((item,i) => (
      ...
      <div style={{ background:'rgba(230,57,70,0.1)', border:'1px solid rgba(230,57,70,0.2)', borderRadius:10, padding:'16px 18px' }}>
        ...
        <div style={{ fontSize:13, color:'rgba(255,255,255,0.65)', lineHeight:1.9 }}>
          ...
          • Passing score: 80 dari 180 poin total<br/>
          • 3 seksi: Moji-Goi (25 mnt) · Chokai (30 mnt) · Bunpou-Dokkai (50 mnt)<br/>
          • Target kosakata: ~800 kata · Target kanji: ~100<br/>
  ...
  return (
    <div style={{ maxWidth:500, margin:'0 auto' }}>
      ...
      <div style={{ textAlign:'center', margin:'30px 0 26px' }}>
        ...
        <div style={{ fontSize:isLong?16:64, fontFamily:'"Noto Serif JP",serif', background:'rgba(255,255,255,0.04)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:14, padding:isLong?'16px 20px':'22px 32px', display:'inline-block', minWidth:120, lineHeight:1.4 }}>
          {q.q}
        </div>
